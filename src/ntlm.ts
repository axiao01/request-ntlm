...
import { IType2 } from "./interface";
// OEM strings are supported for use in security buffer data. */
...
const createBasicMessage = (user: string, pwd: string): string => {
  return "Basic " + Buffer.from(user + ":" + pwd, "utf8").toString("base64");
};

const createType1Message = (
...
const decodeType2Message = (str: any): IType2 => {
  if (str === undefined) {
